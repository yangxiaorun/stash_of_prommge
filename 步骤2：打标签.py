...
while id_date <= 1:
    ...
    print('第%d天的数据'%id_date)
    for driver_id in driver:
        driver_id = driver_id.rstrip('\n')
        with open('E:/滴滴数据处理程序/司机订单大于20的集合/10-'+str(id_date)+'/司机' + driver_id + '.csv', mode='r', encoding='utf-8') as f:
            ...
            df.to_csv('标签_订单大于20的司机.csv', mode='w', index=False, header=False)
    id_date += 1
